...
| :--- | :--- | :--- | :--- |
| 2026-03-31 | 1.10 | Initial analysis document: use case, domain analysis, and UI prototype sections | 안효원 |
| 2026-04-04 | 1.20 | Detailed use case descriptions and scenario flows | 안효원 |
| 2026-04-22 | 1.30 | Refined UML notation and use case diagram consistency | 안효원 |
| 2026-05-01 | 1.31 | Standardized actor terminology based on system boundary rules | 안효원 |
| 2026-05-05 | 1.32 | Strengthened domain model, UC scenarios, business rules, glossary, and references | 안효원 |
| 2026-05-08 | 1.34 | Reorganized Chapter 3; unified report tone, terminology, and caption formatting | 안효원 |
| 2026-05-22 | 1.40 | Added UC-00 (GitHub OAuth 2.0), User domain class, and authentication flow integration | 안효원 |
| 2026-05-23 | 1.41 | Security controls, error handling, PR/Issue data collection, endpoint authentication, PostgreSQL persistence | 안효원 |
| 2026-05-24 | 1.42 | UC list synced with Conceptualization; deployment environment aligned to Ubuntu 22.04+ / WSL2 | 안효원 |
| 2026-05-27 | 1.43 | UC-07 updated to PDF output (OpenPDF); Glossary and Result Dashboard description aligned | 안효원 |
...
- In Scope: GitHub ID 입력부터 데이터 수집, 지표 계산, 점수/피드백 생성, 결과 조회/비교/리포트(PDF) 다운로드까지의 기능 흐름 정의
...
**보안 및 인증 강화:**
- GitHub OAuth 2.0 Authorization Code + state 검증 기반 사용자 인증
...
![Use Case Diagram](../assets/images/UseCaseDiagram.png)

[그림 2-1] Use case diagram

> **Note:** PNG 이미지 렌더링이 불가능한 경우, PlantUML 소스([UseCaseDiagram.jar](../assets/UseCaseDiagram.jar))를 로컬에서 실행하여 다이어그램을 생성할 수 있습니다.  
> 실행 명령: `java -jar plantuml.jar ../assets/UseCaseDiagram.jar`
...
| UC-07 | UC-06 | «extend» | 결과 조회 화면에서 리포트 다운로드 선택적 실행 |
...
| 2 | System이 OAuth state를 생성/저장한다. |
...
| Security | OAuth state 단일 사용, CSRF 검증, 인증 코드 단일 사용, Access Token 안전 보관 |
| Due Date | N/A |
...
| 항목 | 내용 |
| :--- | :--- |
| Summary | 사용자가 분석 결과 리포트(PDF)를 생성하고 다운로드한다. |
| Scope | GitHub Activity Insight |
| Level | User Level |
| Author | 안효원 |
| Last Update | 2026-04-28 |
...
| Primary Actor | User |
| Secondary Actor | (None) |
| Trigger | actor가 Result Dashboard에서 [리포트 다운로드] 버튼을 클릭한다. |
| Preconditions | system이 AnalysisResult 완료 상태를 확인하고 렌더링 엔진이 정상 작동함을 확인한다. |
| Success Post Condition | system이 리포트(PDF) 다운로드를 완료하고 actor가 공유/보관 가능한 문서를 확보한다. |
| Failed Post Condition | system이 리포트 생성 또는 다운로드 실패를 처리하고 actor에게 대체 수단을 제시한다. |

##### UC-07 MAIN SUCCESS SCENARIO

| Step | Action |
| :--- | :--- |
| S | User가 분석 결과 리포트를 다운로드한다. |
| 1 | System이 현재 AnalysisResult 데이터를 확인한다. |
| 2 | System이 AnalysisResult를 ReportAssembler로 전달해 리포트 모델(Map)로 변환한다. |
| 3 | System이 ReportGenerator에 PDF 리포트 렌더링을 요청한다. |
| 4 | System이 totalScore, Metrics, FeedbackItem, ruleVersion을 모델에 바인딩한다. |
| 5 | System이 타임스탬프, GitHub ID, 분석 버전 정보를 포함한다. |
| 6 | System이 OpenPDF를 사용해 A4 PDF 바이트를 생성한다. |
| 7 | System이 파일명을 설정한다(report_{githubId}_{yyyyMMdd_HHmmss}.pdf). |
...
| 4b.3 | System이 PDF 리포트 생성을 계속 진행한다. |
...
| 항목 | 내용 |
| :--- | :--- |
| Performance | 1-3초 (PDF 렌더링 및 다운로드 응답) |
...
| UC-07 리포트 다운로드 | Medium | 결과 공유 및 보관 편의성을 높이는 부가 기능이다. |
...
![그림 4-1 Home Start Screen](../assets/images/ui/4-1-home-start.png)
...
![그림 4-2 Analysis Progress](../assets/images/ui/4-2-analysis-progress.png)
...
종합 역량 점수와 4개 세부 지표인 활동성, 다양성, 협업도, 지속성을 단일 화면에서 통합적으로 제공하며, 강점과 개선안 역시 함께 제시한다. 데이터 신뢰도가 낮은 경우에는 결과 상단에 신뢰도 경고 배지를 표시하여 해석상 주의가 필요함을 알린다. 또한 사용자는 리포트 다운로드(PDF) 버튼을 통해 결과를 문서로 저장할 수 있고, 이력 비교 버튼을 통해 History Compare 화면으로 이동할 수 있다.

![그림 4-3 Result Dashboard](../assets/images/ui/4-3-result-dashboard.png)
...
![그림 4-4 History Compare](../assets/images/ui/4-4-history-compare.png)
...
| Report Generator | 결과를 대시보드 또는 PDF 리포트(OpenPDF A4)로 변환/출력하는 모듈 |
| Rate Limit | GitHub API 호출 횟수 제한 정책으로, 초과 시 일정 시간 요청이 제한되는 제약 |
| RateLimit Reset Handling | API 429 응답의 X-RateLimit-Reset 헤더를 기반으로 재시도 가능 시각을 계산하는 처리 전략 |
