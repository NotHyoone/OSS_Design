...
- In Scope: GitHub ID 입력부터 데이터 수집, 지표 계산, 점수/피드백 생성, 결과 조회/비교/리포트(JSON) 다운로드까지의 기능 흐름 정의
...
| 항목 | 내용 |
| :--- | :--- |
| Summary | 사용자가 분석 결과 리포트(JSON)를 생성하고 다운로드한다. |
| Scope | GitHub Activity Insight |
| Level | User Level |
| Author | 안효원 |
| Last Update | 2026-04-28 |
...
| Success Post Condition | system이 리포트(JSON) 다운로드를 완료하고 actor가 공유/보관 가능한 문서를 확보한다. |
...
| 3 | System이 ReportGenerator에 JSON 리포트 렌더링을 요청한다. |
| 4 | System이 totalScore, Metrics, FeedbackItem, ruleVersion을 모델에 바인딩한다. |
| 5 | System이 타임스탬프, GitHub ID, 분석 버전 정보를 포함한다. |
| 6 | System이 JSON 리포트 바이트를 생성한다. |
| 7 | System이 파일명을 설정한다(report_{githubId}_{yyyyMMdd_HHmmss}.json). |
...
| 4b.3 | System이 JSON 리포트 생성을 계속 진행한다. |
...
| 항목 | 내용 |
| :--- | :--- |
| Performance | 1-3초 (JSON 직렬화 및 다운로드 응답) |
...
종합 역량 점수와 4개 세부 지표인 활동성, 다양성, 협업도, 지속성을 단일 화면에서 통합적으로 제공하며, 강점과 개선안 역시 함께 제시한다. 데이터 신뢰도가 낮은 경우에는 결과 상단에 신뢰도 경고 배지를 표시하여 해석상 주의가 필요함을 알린다. 또한 사용자는 리포트 다운로드(JSON) 버튼을 통해 결과를 문서로 저장할 수 있고, 이력 비교 버튼을 통해 History Compare 화면으로 이동할 수 있다.
...
| Report Generator | 결과를 대시보드 또는 JSON 리포트로 변환/출력하는 모듈 |
