...
| 이름 | 설명 |
| :--- | :--- |
| User(사용자) | 시스템을 이용하는 사용자 클래스이다. GitHub 활동 분석을 요청하고, 자신의 분석 결과와 이력을 조회하는 주체를 의미한다. |
| AnalysisTarget(분석 대상) | 분석 대상이 되는 GitHub 계정을 나타내는 클래스이다. GitHub ID, 프로필 주소, 검증 여부를 속성으로 가지며 외부 활동 데이터 수집의 기준이 된다. |
| AnalysisSession(분석 세션) | 1회의 분석 실행을 나타내는 클래스이다. 요청 시각과 처리 상태를 관리하며, 데이터 수집부터 결과 생성까지의 분석 수명주기를 표현한다. |
...
| 규칙 ID | 제약/규칙 |
| :--- | :--- |
| BR-01 | AnalysisTarget을 식별하는 GitHub ID는 공백이 아닌 영문/숫자/하이픈 조합이어야 한다. |
| BR-02 | AnalysisSession의 상태는 PENDING -> RUNNING -> COMPLETED/FAILED/PARTIAL 상태 전이 규칙을 따른다. (UC-03 확장 시나리오에서 PARTIAL 상태 발생 가능) |
| BR-03 | 동일 사용자의 RUNNING 상태 요청은 최대 1건으로 제한한다(중복 분석 방지). |
| BR-04 | InsightReport의 총점은 0~100 범위를 벗어날 수 없다. |
| BR-05 | EvaluationModel의 입력 근거가 임계치 미만인 경우 결과에 신뢰도 경고를 포함해야 한다. 구체적 임계치: 커밋 데이터 30개 미만, 활동 기간 3개월 미만, 저장소 1개 미만인 경우 신뢰도를 LOW로 표시. |
| BR-06 | 하나의 InsightReport는 반드시 1개의 EvaluationModel 해석 결과와 1개 이상의 Feedback을 포함해야 한다. |
| BR-07 | 사용자별 분석 이력은 InsightReport.createdAt 기준으로 누적 저장되어야 하며, UC-08 비교는 동일 사용자 소유 결과끼리만 허용한다. |
| BR-08 | AnalysisSession이 FAILED 또는 PARTIAL 상태로 종료된 경우, InsightReport에는 데이터 제한 또는 실패 사유가 추적 가능하게 기록되어야 한다. |
