...
classDiagram
    ...
    class MetricCalculator {
        ...
        +filterOutliers(List~Float~ scores) List~Float~
        +detectAnomalies(ActivityData data, Metrics target) Boolean
        -calculateTimeConsistency(List~CommitData~) Float
    ...
    class CompetencyScorer {
        ...
        +getAppliedWeights() Map~String_Float~
    }

    class FeedbackGenerator {
        -Map~String_List~ feedbackRuleTable
        ...
        +validateWeaknesses(List~String~) Boolean
        -mapWeaknessToAction(String category) FeedbackItem
        -rankByPriority(List~FeedbackItem~) List~FeedbackItem~
        -selectTopN(List~FeedbackItem~, Integer n) List~FeedbackItem~
    }

    class AnalysisService {
        -Integer executionTimeout
        -Integer maxRetries
        +requestAnalysis(String githubId) AnalysisRequest
        +executeAnalysis(String requestId) void
        +retryFailedAnalysis(String requestId) void
        +getResult(String requestId) AnalysisResult
        +getHistory(String githubId) List~AnalysisResult~
    ...
    class AnalysisRepository {
        ...
        +findAll() List~AnalysisRequest~
        +findByStatus(RequestStatus status) List~AnalysisRequest~
        +saveResult(AnalysisResult result) void
        +findResultByRequestId(String requestId) AnalysisResult
        +findResultsByGithubId(String githubId) List~AnalysisResult~
        +findAllResults() List~AnalysisResult~
        +deleteExpired(Integer ttlMinutes) Integer
        +getStatistics() Map~String_Object~
    }
    ...
    class ReportAssembler {
        ...
        -calculateProgressTrend(List~AnalysisResult~) Map~String_Float~
    }
...
| 이름 | 시그니처 | 반환 타입 | 설명 |
| :--- | :--- | :---: | :--- |
| requestAnalysis | requestAnalysis(String githubId) | AnalysisRequest | GithubProfile 유효성 검증 후 AnalysisRequest(status=PENDING)를 생성하여 AnalysisRepository에 저장하고 반환한다. |
...
| getHistory | getHistory(String githubId) | List\<AnalysisResult\> | AnalysisRepository.findResultsByGithubId()로 해당 사용자의 전체 분석 이력을 반환한다. UC-08 지원. |
...
| findResultsByGithubId | findResultsByGithubId(String githubId) | List\<AnalysisResult\> | 해당 githubId의 모든 AnalysisResult를 createdAt 내림차순으로 반환한다. 이력 비교(UC-08)에 사용. |
